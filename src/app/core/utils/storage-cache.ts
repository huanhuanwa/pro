...
const cache = {
    ...
    set<TValue = string>(key: string, value: TValue, compress = true) {
        let itemValue = lodash.isString(value) ? value : lodash.isNumber(value) ? `${NUMBER_PREFIX}${value}` : JSON.stringify(value);
        if (compress) {
        ...
        if (SupportedStorage) {
            ...
            if (value) {
                ...
                try {
                    ...
                    if (lodash.isString(value) && value.includes(NUMBER_PREFIX)) {
